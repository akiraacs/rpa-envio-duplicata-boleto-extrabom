...
class CaminhosSettings(BaseSettings):
    ...
    arquivo_datas_execucao: str = f"{onedrive_robo}\datas_execucao.json"


    @field_validator("arquivo_datas_execucao", mode="before")
    @classmethod
    def validar_arquivo(cls, v: str) -> str:
        if not v:
            return cls.model_fields["arquivo_datas_execucao"].default
        return v
